...
export default class Handler extends Namespace.Class {
  ...
  /** @static @function clear()
    ...
    **/
  static clear() {
    if(log) log.debug(`${this.name} clears all handlers`);
    this.stack = new Set();
  }
  ...
  static use(handler) {
    ...
    if(log) log.debug(`${this.name} uses ${handler}`);
  ...
  clear(handler = undefined) {
    if(handler === undefined) {
      ...
      if(log) log.debug(`${this.name} removes handler ${handler}`);
  ...
  invoke(request, response, data = {}) {
    if(log) log.debug({ message: `${this.name} invokes handler`, source: this.name });
    return new Promise((resolve, reject) => {
      // Load data
