...
class Namespace {
  ...
  static load(registry = this.#registry) {
    return new Promise((resolve, reject) => {
      ...
      Promise.allSettled(promises)
        .then(() => {
          registry = {};
          resolve({ succeeded: this.#succeeded, failed: this.#failed, omitted: this.#omitted });
        });
    });
    return this;
  }
  ...
  static registerPath(searchPath, dependency = undefined) {
    return new Promise((resolve, reject) => {
      let promises = [];
      try {
        let entries = Deno.readDirSync(searchPath);
        for(const entry of entries) {
          let baseName = entry.name.substr(0, entry.name.lastIndexOf('.'));
          let extension = entry.name.substr(baseName.length);
          if(entry.isDirectory) {
            promises.push(this.registerPath(`${searchPath}/${entry.name}`, entry.name));
          } else if(this.default.includeExtensions.includes(extension)) {
            this.registerModule({ name: baseName, path: `${searchPath}/${entry.name}`, dependency: dependency })
          }
        }
      } catch(error) {
        reject(error);
  ...
  /** @static @function resolvePath(path,basePath)
